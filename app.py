...
def get_server_or_404(server_id):
    """Возвращает сервер, если пользователь является владельцем или совладельцем."""
...
@login_required
def index():
    # Серверы, где пользователь является владельцем или совладельцем
    servers = Server.query.filter(
        (Server.user_id == current_user.id) |
        (Server.co_owners.any(id=current_user.id))
    ).all()
    ...
    db.session.commit()
    # Передаём в шаблон список кортежей (сервер, статистика)
...
def api_server_stats(server_id):
    """Возвращает системную статистику для сервера."""
    server = get_server_or_404(server_id)
    try:
        client = ssh_connect(server.ssh_host, server.ssh_port, server.ssh_user, server.get_password())
        stats = get_system_stats(client)
...
def api_get_logs(server_id):
    ...
    try:
        ...
        return jsonify({'logs': f'Error: {e}'}), 500
